...
namespace libcmaes
{
  ...
  CMAParameters::CMAParameters(const int &dim, const int &lambda,
			       ...
			       const uint64_t &seed)
    :Parameters(dim,lambda,max_iter,max_fevals,x0,fplot,seed),_sigma_init(sigma_init),_nrestarts(9),_lazy_update(false),_lazy_value(0),_cm(1.0),_alphacov(2.0),_alphaminusold(0.5),_lambdamintarget(0.66),_alphaminusmin(1.0)
  {
